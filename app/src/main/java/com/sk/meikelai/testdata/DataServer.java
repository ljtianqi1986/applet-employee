package com.sk.meikelai.testdata;
...
public class DataServer {
    ...
    public static List<Status> getSampleData(int lenth) {
        ...
        for (int i = 0; i < lenth; i++) {
            ...
            status.setUserName("光疗甲一年" + i);
            status.setCreatedAt("甲油胶298，指甲护理20，甲油胶60，甲油胶180");
            status.setRetweet(i % 2 == 0);
            status.setUserAvatar("有效期限1年");
            status.setText("400" + i);
            list.add(status);
        }
        return list;
    }

    public static List<Status> getSampleData2(int lenth) {
        List<Status> list = new ArrayList<>();
        for (int i = 0; i < lenth; i++) {
            Status status = new Status();
            status.setUserName("面部黑脸娃娃110元/次" + i);
            status.setCreatedAt("剩余：9次");
            status.setRetweet(i % 2 == 0);
            status.setUserAvatar("编号：Z403-" + i);
            status.setText("拼音码：MBHLWW" + i);
            list.add(status);
        }
        return list;
    }

    public static List<Status> getSampleData3(int lenth) {
        List<Status> list = new ArrayList<>();
        for (int i = 0; i < lenth; i++) {
            Status status = new Status();
            status.setUserName("储值卡" + i);
            status.setText("余额" + i + "00");
            list.add(status);
        }
        return list;
    }
...
//        list.add(new MySection(new MediaStore.Video(HTTPS_AVATARS1_GITHUBUSERCONTENT_COM_LINK, CYM_CHAD)));
//        list.add(new MySection(new MediaStore.Video(HTTPS_AVATARS1_GITHUBUSERCONTENT_COM_LINK, CYM_CHAD)));
//        list.add(new MySection(new MediaStore.Video(HTTPS_AVATARS1_GITHUBUSERCONTENT_COM_LINK, CYM_CHAD)));
//        list.add(new MySection(new MediaStore.Video(HTTPS_AVATARS1_GITHUBUSERCONTENT_COM_LINK, CYM_CHAD)));
