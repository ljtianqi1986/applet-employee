...
public class CardOnceAdapter extends BaseQuickAdapter<OpenCardBean.ReturnDataBean, BaseViewHolder> {

    public CardOnceAdapter(@LayoutRes int layoutResId, @Nullable List<OpenCardBean.ReturnDataBean> data) {
    ...
    protected void convert(BaseViewHolder helper, OpenCardBean.ReturnDataBean item) {
        helper.setVisible(R.id.item_line, true)
                ...
                .setTextColor(R.id.item_goods, mContext.getResources().getColor(R.color.once_purple))//设置项目名称颜色
                .setTextColor(R.id.item_term, mContext.getResources().getColor(R.color.dark_purple))//设置有效期颜色
//                .setText(R.id.item_off, String.valueOf((int) (item.getDiscount()) + "折")) //折扣
                ...
                .setImageResource(R.id.item_img, R.drawable.cd_zis)          //设置左边卡类型
        ...
        for (OpenCardBean.ReturnDataBean.ProjectListBean moreDataBean : item.getProject_list()) {
            ...
            sb.append(moreDataBean.getTimes()).append("次 , ");
        }
        sb.toString();
        if (sb.length() > 0) {
