...
public class WorkerLoginActivity extends BaseActivity implements HandleDataCallBack {
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_login:
                ...
                if (TextUtils.isEmpty(workername)) {
                    Toast.makeText(mContext, getString(R.string.input_username_hint), Toast.LENGTH_SHORT).show();
                    return;
                }
                if (TextUtils.isEmpty(pwd)) {
                    Toast.makeText(mContext, getString(R.string.input_password_hint), Toast.LENGTH_SHORT).show();
                    return;
                }
                loadingDialog.show();
                AppApi.login(workername, pwd, device_ver, device_info, device_ip, WorkerLoginActivity.this, 0);
                break;
        }
    }
    ...
    public void handlerSuccessData(String result, int tag) {
        ...
        if (tag == 0) {
            ...
            AppApi.getWorkerList(MyUtils.getShopCode(mContext), WorkerLoginActivity.this, 1);
        } else if (tag == 1) {
