...
import com.sk.meikelai.entity.OpenCardPrint;
...
public class CardSuccessActivity extends BaseActivity implements HandleDataCallBack, IsPrintCallBack {
    ...
    protected int getContentView() {
        return R.layout.fragment_card_success;
    }
    ...
    @OnClick({R.id.back, R.id.go_cash_f, R.id.go_card_f, R.id.quit_account_f})
    ...
    public void handlerSuccessData(String result, int tag) {
        ...
        OpenCardPrint openCardPrint = JSON.parseObject(result, OpenCardPrint.class);
        String order_code = openCardPrint.getReturn_data().getOrder_code();
        String order_time = openCardPrint.getReturn_data().getOrder_time();
        String cash_pep = openCardPrint.getReturn_data().getPerson_name();
        String base_user_name = openCardPrint.getReturn_data().getBase_user_name();
        ...
        String  card_name = openCardPrint.getReturn_data().getCard_name();
        int  should_money = openCardPrint.getReturn_data().getShould_money();
        String  useful_date = openCardPrint.getReturn_data().getUseful_date();
        stringBuilder.append("\n").append("名称:").append(card_name).append("\n");
        stringBuilder.append("售价:").append(should_money/100f).append("元\n");
        stringBuilder.append("有效期至:").append(useful_date).append("\n");


        stringBuilder.append("\n应收金额:").append(String.valueOf(openCardPrint.getReturn_data().getShould_money()/100f)).append("元").append("\n");
        stringBuilder.append("实售金额:").append(String.valueOf(openCardPrint.getReturn_data().getActual_money()/100f)).append("元").append("\n");
        int cash_total = openCardPrint.getReturn_data().getActual_money();
        if(openCardPrint.getReturn_data().getTrade_type().equals("CASH")){
            stringBuilder.append("现金支付:").append(cash_total/100f).append("元").append("\n");
        }else if(openCardPrint.getReturn_data().getTrade_type().equals("ONLINE")){
            stringBuilder.append("电子支付:").append(cash_total/100f).append("元").append("\n");
        }else if(openCardPrint.getReturn_data().getTrade_type().equals("CARD")){
            stringBuilder.append("银行卡付款:").append(cash_total/100f).append("元").append("\n");
        }
        String remark = openCardPrint.getReturn_data().getRemark();
        ...
        String phone = openCardPrint.getReturn_data().getShop_telephone();
        String address = openCardPrint.getReturn_data().getShop_address();
        stringBuilder.append("\n门店电话:").append(phone);
        stringBuilder.append("\n门店地址:").append(address);
        ...
        if (Build.MANUFACTURER.contains("basewin")) {
            ...
            isPrintDialogFragment.setCallBack(CardSuccessActivity.this);
